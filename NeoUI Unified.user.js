...
// ==============================================================================
// MODULE 4: ITEM TRANSFER LOG (REFORMATTED)
// ==============================================================================

(function () {
  'use strict';

  const STYLE_ID = 'ext1nct-tlog-style';
  ...
  function injectStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      /* ---- Reclaim mobile width from the legacy page chrome ----
         NOTE: .premium_bar_spacer is NOT a small ad spacer on this template -
         it's the class on the #main wrapper that contains the whole page
         (header, content, footer). Hiding it hides everything, including
         the cards we're about to build. Only hide the specific legacy
         pieces we actually mean to replace: the old nav header (replaced
         by our topbar), the old footer, the sidebar column, and the
         premium ad dock - never #main/.premium_bar_spacer itself. */
      #header, #footer, #ban, #premium_bar, .nav-premium-container,
      #premium-bar, .premium-toolbar, #superfooter, #footer_menu,
      .sswdrop, .featureddrop, .dailiesdrop, .snapshotdrop,
      td.sidebar, #ads, .adsWrapper {
        display: none !important;
      }
      ...
      /* #main/#content on this template are boxed into a fixed, centered
         width (originally to leave room for skyscraper ad slots down each
         side) - with the ads hidden that just reads as two dead vertical
         gutters, at a fixed pixel width that eats an even bigger share of
         a phone screen. Force them full-bleed instead. */
      html[data-neoui-theme] #main,
      html[data-neoui-theme] #content,
      html[data-neoui-theme] #content > table,
      html[data-neoui-theme] td.content {
        width: 100% !important;
      ...
      .ext1nct-page-title {
        ...
        font-weight: 800;
        text-align: center;
        margin: var(--nui-space-2) auto var(--nui-space-3);
        padding: 0 var(--nui-space-4);
      }

      .ext1nct-orig-pagination { display: none !important; }
      ...
      /* Mobile-first: image + info on top row, action pills fill a second
         full-width row underneath instead of squeezing into the same row
         (which used to leave the buttons cramped and the row itself
         needing extra height it didn't have — reading as "blank space"
         above/below the card on narrow screens). */
      .ext1nct-card {
        border: 1px solid var(--nui-border);
        ...
        grid-template-areas:
          "img info"
          "actions actions";
        column-gap: var(--nui-space-3);
        row-gap: var(--nui-space-3);
        align-items: center;
        box-sizing: border-box;
      }

      /* Tablet/desktop: collapse back to a single tidy row since there's
         room for the pills beside the item info. */
      @media (min-width: 640px) {
        ...
        .ext1nct-card-actions { width: auto; }
        .ext1nct-card-actions label { flex: 0 0 auto; }
      }
      ...
      .ext1nct-from-link:hover {
        text-decoration: underline;
      }

      .ext1nct-card-actions {
        grid-area: actions;
        display: flex;
        flex-direction: row;
        width: 100%;
      ...
      .ext1nct-card-actions label {
        ...
        background: var(--nui-surface-2);
        color: var(--nui-text-muted);
        transition: background var(--nui-dur-fast) var(--nui-ease), border-color var(--nui-dur-fast) var(--nui-ease), color var(--nui-dur-fast) var(--nui-ease);
        user-select: none;
        text-align: center;
      }
      .ext1nct-card-actions label:active {
        transform: scale(0.97);
      }
      .ext1nct-card-actions input[type="radio"] {
        position: absolute;
        opacity: 0;
        width: 0;
        height: 0;
      ...
      .ext1nct-card-actions label.ext1nct-accept-label:has(input:checked),
      .ext1nct-card-actions label.ext1nct-accept-label.ext1nct-checked {
        background: var(--nui-success);
        border-color: var(--nui-success);
        color: var(--nui-surface);
      }
      .ext1nct-card-actions label.ext1nct-return-label:has(input:checked),
      .ext1nct-card-actions label.ext1nct-return-label.ext1nct-checked {
        background: var(--nui-danger);
        border-color: var(--nui-danger);
        color: var(--nui-surface);
      }
      .ext1nct-card-actions label.ext1nct-discard-label:has(input:checked),
      .ext1nct-card-actions label.ext1nct-discard-label.ext1nct-checked {
        background: var(--nui-text-muted);
        border-color: var(--nui-text-muted);
        color: var(--nui-surface);
      }
      ...
      .ext1nct-toolbar button.ext1nct-danger {
        background: var(--nui-danger-soft);
        border-color: var(--nui-danger-soft);
      ...
      .ext1nct-banner {
        ...
        margin: var(--nui-space-3) auto;
        padding: var(--nui-space-3) var(--nui-space-4);
        background: var(--nui-warning-soft);
        border: 1px solid var(--nui-warning);
        border-radius: var(--nui-radius-md);
        font-family: var(--nui-font-body);
        font-size: 14px;
        color: var(--nui-text);
  ...
  const AUTOACCEPT_MAX_PAGES = 250;
  ...
  function buildPagination() {
    ...
    const wrap = document.createElement('div');
    wrap.className = 'ext1nct-pager';

    const prevBtn = document.createElement('button');
    prevBtn.type = 'button';
    prevBtn.textContent = '\u2039 Prev';
    prevBtn.disabled = currentPage <= 1;
    prevBtn.addEventListener('click', () => navigateToPage(Math.max(1, currentPage - 1)));

    const select = document.createElement('select');
    for (let i = 1; i <= maxPage; i++) {
      const opt = document.createElement('option');
      opt.value = i;
      opt.textContent = `Page ${i} / ${maxPage}`;
      if (i === currentPage) opt.selected = true;
      select.appendChild(opt);
    }
    select.addEventListener('change', () => navigateToPage(parseInt(select.value, 10)));

    const nextBtn = document.createElement('button');
    nextBtn.type = 'button';
    nextBtn.textContent = 'Next \u203a';
    nextBtn.disabled = currentPage >= maxPage;
    nextBtn.addEventListener('click', () => navigateToPage(Math.min(maxPage, currentPage + 1)));

    wrap.appendChild(prevBtn);
    wrap.appendChild(select);
    wrap.appendChild(nextBtn);

    return { element: wrap, anchorTable: pagerTable };
  }
  ...
  function syncCheckedLabel(radio) {
    const row = radio.closest('.ext1nct-card-actions');
    if (!row) return;
    row.querySelectorAll('label').forEach((l) => l.classList.remove('ext1nct-checked'));
    radio.closest('label')?.classList.add('ext1nct-checked');
  }
  ...
  function buildCards() {
    ...
    acceptAllBtn.addEventListener('click', () => {
      document.querySelectorAll('input.np_sel_radio, input.nc_gift_sel_radio').forEach((r) => {
        r.checked = true;
        syncCheckedLabel(r);
      });
    });
    ...
    returnAllBtn.textContent = 'Return All';
    returnAllBtn.addEventListener('click', () => {
      document.querySelectorAll('input.np_reject_all, input.nc_gift_rej_radio').forEach((r) => {
        r.checked = true;
        syncCheckedLabel(r);
      });
    });

    const acceptAllPagesBtn = document.createElement('button');
    acceptAllPagesBtn.type = 'button';
    acceptAllPagesBtn.className = 'ext1nct-danger';
    acceptAllPagesBtn.textContent = 'Accept All (All Pages)';
    acceptAllPagesBtn.addEventListener('click', () => {
      const confirmed = window.confirm(
        'This will accept EVERY item across ALL forms and pages of your transfer log, running in the background. This cannot be undone. Continue?'
      );
      if (!confirmed) return;

      autoAcceptCancelled = false;
      runAutoAcceptInIframe();
    });

    toolbar.appendChild(acceptAllBtn);
    toolbar.appendChild(returnAllBtn);
    toolbar.appendChild(acceptAllPagesBtn);
    ...
    tables.forEach((table) => {
      ...
      cardsWrap.className = 'ext1nct-cards';
      ...
      rows.forEach((row) => {
        ...
        if (isNC) {
          ...
          itemCell = itemCellRef;
          acceptCell = acceptCellRef;
          rejectCell = rejectCellRef;
          discardCell = discardCellRef;
        ...
        info.className = 'ext1nct-card-info';

        const itemNameDiv = document.createElement('div');
        itemNameDiv.className = 'ext1nct-card-item';
        itemNameDiv.textContent = itemName;

        const metaDiv = document.createElement('div');
        metaDiv.className = 'ext1nct-card-meta';
        metaDiv.appendChild(document.createTextNode('From '));
        if (fromHref) {
          const fromAnchor = document.createElement('a');
          fromAnchor.href = fromHref;
          fromAnchor.className = 'ext1nct-from-link';
          fromAnchor.textContent = fromName;
          metaDiv.appendChild(fromAnchor);
        } else {
          metaDiv.appendChild(document.createTextNode(fromName));
        }
        metaDiv.appendChild(document.createTextNode(` \u00b7 ${dateText}`));

        info.appendChild(itemNameDiv);
        info.appendChild(metaDiv);

        const actions = document.createElement('div');
        actions.className = 'ext1nct-card-actions';

        const acceptRadio = acceptCell ? acceptCell.querySelector('input[type="radio"]') : null;
        const rejectRadio = rejectCell ? rejectCell.querySelector('input[type="radio"]') : null;
        const discardRadio = discardCell ? discardCell.querySelector('input[type="radio"]') : null;

        if (acceptRadio) {
          const lbl = document.createElement('label');
          lbl.className = 'ext1nct-accept-label';
          lbl.appendChild(acceptRadio);
          lbl.appendChild(document.createTextNode('Accept'));
          actions.appendChild(lbl);
        }
        if (rejectRadio) {
          const lbl = document.createElement('label');
          lbl.className = 'ext1nct-return-label';
          lbl.appendChild(rejectRadio);
          lbl.appendChild(document.createTextNode('Return'));
          actions.appendChild(lbl);
        }
        if (discardRadio) {
          const lbl = document.createElement('label');
          lbl.className = 'ext1nct-discard-label';
          lbl.appendChild(discardRadio);
          lbl.appendChild(document.createTextNode('Discard'));
          actions.appendChild(lbl);
        }

        [acceptRadio, rejectRadio, discardRadio].forEach((radio) => {
          if (!radio) return;
          radio.addEventListener('change', () => syncCheckedLabel(radio));
          if (radio.checked) syncCheckedLabel(radio);
        });
  ...
  function runAutoAcceptInIframe() {
    ...
    function step() {
      if (autoAcceptCancelled) {
        showAutoAcceptBanner('Stopped.', true);
        cleanup();
        return;
      }

      if (formsProcessed >= AUTOACCEPT_MAX_PAGES) {
        showAutoAcceptBanner(
          `Stopped after ${AUTOACCEPT_MAX_PAGES} submits as a safety limit. Click "Accept All (All Pages)" again to continue.`,
          true
        );
        cleanup();
        return;
      ...
      try {
        ...
        showAutoAcceptBanner('Could not read the background page — stopped.', true);
        cleanup();
        return;
      }
      ...
      if (acceptRadios.length === 0) {
        showAutoAcceptBanner(`Done — accepted items across ${formsProcessed} form(s).`, true);
        cleanup();
        return;
      }

      const formToSubmit = acceptRadios[0].closest('form');
      const radiosInForm = formToSubmit.querySelectorAll('input.np_sel_radio, input.nc_gift_sel_radio');

      radiosInForm.forEach((r) => (r.checked = true));
      formsProcessed += 1;

      showAutoAcceptBanner(`Accepting in the background... (${formsProcessed} form(s) submitted so far)`, false);
  ...
  function showAutoAcceptBanner(message, finished) {
    let banner = document.querySelector('.ext1nct-banner');

    if (!banner) {
      banner = document.createElement('div');
      banner.className = 'ext1nct-banner';
      const firstToolbar = document.querySelector('.ext1nct-toolbar');

      if (firstToolbar) {
        firstToolbar.insertAdjacentElement('afterend', banner);
      } else {
  ...
  function injectPageTitle(anchor) {
    ...
    title.className = 'ext1nct-page-title';
    title.textContent = 'Item Transfer Log';
  ...
  function reformatPage() {
    // Scrape the real username/pet/NP/NC off the legacy header BEFORE
    // touching anything, same as every other module - so the shared topbar
    // and drawer show real data instead of placeholders.
    const NeoUI = window.NeoUI;
    let profile = null;
    if (NeoUI && NeoUI.__ready) {
      profile = NeoUI.scrapeLegacyProfile();
    }
    ...
    // (the actual cause of the old dead space on phones), then use the
    // exact same topbar every other page uses - not a bespoke one.
    if (NeoUI && NeoUI.__ready) {
      NeoUI.init();
      if (profile) NeoUI.setProfileInfo(profile);
      NeoUI.buildTopbar({ stats: profile ? { np: profile.np, nc: profile.nc } : undefined });
    } else if (NeoUI && NeoUI.injectViewport) {
      NeoUI.injectViewport();
    }

    let pagerEl = null;
    if (!document.querySelector('.ext1nct-pager')) {
      const pager = buildPagination();
      if (pager && pager.anchorTable) {
        pager.anchorTable.insertAdjacentElement('afterend', pager.element);
        pagerEl = pager.element;
      }
    }
  ...
  if (/\/items\/transfer_list\.phtml/.test(location.pathname)) {
    const start = function () {
      try { reformatPage(); } catch (err) { showFatalError(err); }
    };
